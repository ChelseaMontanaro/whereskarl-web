...
import { BAY_AREA_MAP_STYLE_URL } from '@/lib/map/mapConfig';
...
/**
 * Approved phone-portrait treatment: deep navy/near-black, high contrast,
 * heavily desaturated so the satellite texture reads as a quiet night map.
 */
const phonePortraitCinematicSatellitePaint = {
  "raster-brightness-min": 0.02,
  "raster-brightness-max": 0.42,
  "raster-contrast": 0.46,
  // Rotate the warm daytime land tones toward deep navy for the approved
  // night-map treatment, then mute what's left.
...
/**
 * Phone-portrait hybrid: quiet roads, dimmed default labels (raster label
 * tiles cannot be filtered per-name, so nonessential labels are suppressed by
 * lowering opacity until only the larger city names read).
 */
